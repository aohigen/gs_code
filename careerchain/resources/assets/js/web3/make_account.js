...
async function func() {
	...
	//トークン送付のための少額のETHを送付
	await web3.eth.sendTransaction({from: coinbase, to: newAddress, value: 1000000000000000000});
    //コントラクトのインスタンスを生成
    const myContract = await new web3.eth.Contract(abi,tokenAddress);
    let tranHash = await myContract.methods.transfer(newAddress,300).send({from: coinbase})
    let getBalance = await myContract.methods.balanceOf(newAddress).call();
    //もともと以下の記述もあったけど、ユーザーにETHを持たせる必要はないからコメントアウト
    // var sendTransaction = await web3.eth.sendTransaction({from:accounts[0],to:newAddress,value:10});
