...
if($edit_password !== $edit_password2){ //パスワードが違ったらDBを読む前にresister.phpに戻す
  ...
  if($status==false) {
    sql_error();
      ...
      //３．データ登録SQL作成
      $stmt = $pdo->prepare("UPDATE user_table SET user_name=:name,email=:email,plan=:plan,password=:password,admin_flg=:admin_flg,status=:status WHERE user_id=:id");
      ...
      $stmt->bindValue(':status', $edit_status, PDO::PARAM_STR);
