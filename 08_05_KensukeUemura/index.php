...
$sqlFrom = "population2";
if($filterDim == null || $filterVal == null){ //いずれかが空なら全てのデータをセット
  $sqlWhereDim = 'age';
  $sqlWhereVal = "'総数'";
}else{
...
$sqlWhereDim2 = $filterDim2;
$sqlWhereVal2 = "'".$filterVal2."'";
$sqlWhereDim3 = $filterDim3;
$sqlWhereVal3 = "'".$filterVal3."'";
...
if(!empty($filterMatchType)){
  ...
  $sqlWhere = $sqlWhereDim."=".$sqlWhereVal;//フィルタをかける前のデフォルト表示用
  }

$sqlReqest = "SELECT {$sqlSelect} FROM {$sqlFrom} WHERE {$sqlWhere} GROUP BY {$sqlGroupby}";

//アドバンスフィルタがあった場合は、$sqlReqestを上書き
if(!empty($filterDim3)){
  $sqlWhere2 = ' AND '.$sqlWhereDim2.$filterMatchType2.$sqlWhereVal2;
  $sqlWhere3 = ' AND '.$sqlWhereDim3.$filterMatchType3.$sqlWhereVal3;
  $sqlWhere = $sqlWhere.$sqlWhere2.$sqlWhere3;
  $sqlReqest = "SELECT {$sqlSelect} FROM {$sqlFrom} WHERE {$sqlWhere} GROUP BY {$sqlGroupby}";
  }
if(!empty($filterDim2)){
  $sqlWhere2 = ' AND '.$sqlWhereDim2.$filterMatchType2.$sqlWhereVal2;
...
// $sqlReqest = "SELECT {$sqlSelect} FROM {$sqlFrom} WHERE {$sqlWhere} GROUP BY {$sqlGroupby}";
//※初めは、上のように一括で$sqlReqestを定義していたが、なぜかアドバンスフィルタの時だけ、var_dampで見ると正しい文字列が出力できているのに、SQLからは何も帰って来なかった。
//※そこで、何日も悩んだが、49行目・54行目の位置にそれぞれ持っていくとうまく動いた。理由は謎のまま。
          ...
          <li class="breadcrumb-menu d-md-down-none">
            ...
            <?php
              ...
              include("parts/filter.php");
                ...
                echo("<span style='color:white;background-color:red;padding:3px 5px 3px 5px;border-radius: 5px 5px 5px 5px'>フィルタ機能は、有料プランでご利用いただけます</span>");
              }
            ?>
            </div>
          </li>
...
//フィルタの表示用。各ディメンションごとに適正な選択肢だけを表示
$('.defaltFilter').change(function(){
...
let filterDim2 = '<?=$filterDim2?>';
let filterMatchType2 = '<?=$filterMatchType2?>';
let filterVal2 = '<?=$filterVal2?>';
let filterDim3 = '<?=$filterDim3?>';
let filterMatchType3 = '<?=$filterMatchType3?>';
let filterVal3 = '<?=$filterVal3?>';
