...
// EXIT("aaaa");
//全ユーザー数の取得
$sqlReqest = "SELECT COUNT('user_id') as num , SUM(crct_times) as crct_num , SUM(answer_times) as answer_num , SUM(quiz_times) as quiz_num FROM user_table";
...
$quiz_num = $user_info["quiz_num"];
$all_crct_rate = round($user_info["crct_num"]/$user_info["answer_num"]*100,2);
...
//ユーザーの回答履歴を取得
$stmt = $pdo->prepare("SELECT * FROM answers ORDER BY answer_time ASC");
$status = $stmt->execute();
if($status==false) {
  sql_error();
}else{
...
//カテゴリごとの成績を取得
$sqlReqest = "SELECT category_id , COUNT('user_id') as num , SUM(crct_flg) as crct_num FROM answers INNER JOIN questions_ohori ON answers.q_id = questions_ohori.q_id INNER JOIN quiz_category ON questions_ohori.category_id = quiz_category.id GROUP BY category_id";
$stmt = $pdo->prepare("$sqlReqest");
$status = $stmt->execute();
if($status==false) {
  sql_error();
...
$cate1_crct_rate = round($category_info[0]["crct_num"]/$category_info[0]["num"]*100,2);
$cate2_crct_rate = round($category_info[1]["crct_num"]/$category_info[1]["num"]*100,2);
$cate3_crct_rate = round($category_info[2]["crct_num"]/$category_info[2]["num"]*100,2);
$cate4_crct_rate = round($category_info[3]["crct_num"]/$category_info[3]["num"]*100,2);
// EXIT(var_dump($category_info[0]["num"]));
?>
...
<html lang="ja">
  <head>
    ...
    <title>OHORI MANIA! -みんなの成績</title>
    <!-- Icons-->
          ...
          <div class="animated fadeIn">
            <div class="row">
              <div class="col-sm-6 col-lg-3">
                <div class="card text-white bg-primary">
                  <div class="card-body pb-0">
                    <div class="text-value recentPop"><?=$user_num?>人</div>
                    <div>総ユーザー数</div>
                  </div>
              ...
              <div class="col-sm-6 col-lg-3">
                <div class="card text-white bg-info">
                  <div class="card-body pb-0">
                    ...
                    <div class="text-value lastRate"><?=$all_crct_rate?>%</div>
                    <div>みんなの正答率</div>
                  </div>
                  <div class="chart-wrapper mt-3 mx-3" style="height:5px;">
              ...
              <div class="col-sm-6 col-lg-3">
                <div class="card text-white bg-danger">
                  <div class="card-body pb-0">
                    <div class="text-value recentWomenRate"><?=$quiz_num?></div>
                    <div>みんなの挑戦数</div>
                  </div>
                  <div class="chart-wrapper mt-3 mx-3" style="height:5px;">
            ...
            <div class="card">
              <div class="card-body">
                <div class="row">
                  <div class="col-sm-5">
                    <h4 class="card-title mb-0">みんなの成績の推移</h4>
              ...
              <div class="card-footer">
                <div class="row text-center">
                  <div class="col-sm-12 col-md mb-sm-2 mb-0">
                    ...
                    <strong><span class="10yNum"><?=$cate1_crct_rate?></span>  %</strong>
                    <div class="progress progress-xs mt-2">
                      <div class="progress-bar bg-info 10yNumBar" role="progressbar" style="width: <?=$cate1_crct_rate?>%" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </div>
                  <div class="col-sm-12 col-md mb-sm-2 mb-0">
                    <div class="text-muted">大堀の歴史</div>
                    <strong><span class="20yNum"><?=$cate2_crct_rate?></span>  % </strong>
                    <div class="progress progress-xs mt-2">
                      <div class="progress-bar bg-warning 20yNumBar" role="progressbar" style="width: <?=$cate2_crct_rate?>%" aria-valuenow="60" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </div>
                  <div class="col-sm-12 col-md mb-sm-2 mb-0">
                    <div class="text-muted">知られざる大堀</div>
                    <strong><span class="50yNum"><?=$cate3_crct_rate?></span>  % </strong>
                    <div class="progress progress-xs mt-2">
                      <div class="progress-bar bg-danger 50yNumBar" role="progressbar" style="width: <?=$cate3_crct_rate?>%" aria-valuenow="80" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </div>
                  <div class="col-sm-12 col-md mb-sm-2 mb-0">
                    <div class="text-muted">G'sでの大堀</div>
                    <strong><span class="75yNum"><?=$cate4_crct_rate?></span> % </strong>
                    <div class="progress progress-xs mt-2">
                      <div class="progress-bar bg-success 75yNumBar" role="progressbar" style="width: <?=$cate4_crct_rate?>%" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                  </div>
                </div>
              </div>
