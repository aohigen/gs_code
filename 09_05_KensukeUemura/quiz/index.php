...
//カテゴリごとの成績を取得
$sqlReqest = "SELECT category_id , COUNT('user_id') as num , SUM(crct_flg) as crct_num FROM answers INNER JOIN questions_ohori ON answers.q_id = questions_ohori.q_id INNER JOIN quiz_category ON questions_ohori.category_id = quiz_category.id WHERE user_id='$user_id' GROUP BY category_id";
$stmt = $pdo->prepare("$sqlReqest");
$status = $stmt->execute();
if($status==false) {
  sql_error();
...
$json_category = json_encode($category_info);
?>
          ...
          <div class="animated fadeIn">
            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="noneGraphAlert">
                  <?php
                  if($user_info["answer_times"]=="0"){
                    include("parts/non_answer.php");
                  }
                  ?>
                </div>
...
let myCategoryArray = JSON.parse('<?=$json_category?>');
console.log(myCategoryArray);
...
let categoryArray = JSON.parse('<?=$category_info?>');
console.log("カテゴリーあれい"+categoryArray);
</script>
